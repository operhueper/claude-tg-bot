...
## Состояние: 2026-05-16 (вечер) — Оптимизация токенов + условный Composio MCP

### Unit-экономика: реальные данные (9 дней, прод)

- Типичный пользователь: $4–5/мес (DeepSeek) → при 499 ₽ ($5.6) ~+$1 маржа
- Тяжёлый пользователь (946882308): $11/мес → убыток -$5
- Корень проблемы: контекст накапливается — средний input 44–57K токенов (должен быть 5–10K)
- Системный промпт был 15K токенов — основной overhead для всех пользователей

### Что сделано за сессию (2 коммита на прод)

| Изменение | Файл | Эффект |
|---|---|---|
| **HISTORY_LIMIT 12→8** | `src/session.ts:355` | -33% токенов из истории диалога |
| **Промпт: рус 400 строк → англ 140 строк** | `src/config.ts:665–802` | 15K → 4K токенов (-73%) |
| **Язык ответа: рус → язык пользователя** | Первые строки промпта | Готов к иностранным юзерам |
| **Авто-подсказка /new после 16 turns** | `src/session.ts:578–585` | Бот сам говорит когда сессия длинная |
| **skills/scheduling.md + daemons.md** | `skills/` | Детали переехали из промпта в skill-файлы |
| **Composio MCP: условная загрузка** | `src/mcp-filter.ts` | 146 тулзов (~50K токенов) грузятся ТОЛЬКО если `googleConnected=true` в users.json |
| **googleConnected флаг в UserNode** | `src/user-registry.ts` | Автоматически ставится при /google OAuth; снимается при disconnect |
| **UserRegistry.saveUser в streaming.ts** | При ACTIVE connections | Флаг персистится, активируется при следующей сессии |
| **Disconnect endpoint → сбрасывает флаг** | `src/dashboard-server.ts` | После отключения Google — MCP не грузится |

### Ожидаемое снижение стоимости

| Пользователь | До | После (промпт+история) | После (+ без Composio) |
|---|---|---|---|
| Типичный (20 msg/день) | ~370 ₽/мес | ~130 ₽/мес | ~80 ₽/мес |
| Тяжёлый (45 msg/день) | ~985 ₽/мес | ~350 ₽/мес | ~200 ₽/мес |

### Коммиты

- `5fd161e` perf: shrink paid system prompt 73%, reduce history limit, auto-suggest /new
- `b7fc57f` perf: load Composio MCP only when user has active Google connections

### Открытые задачи

- Ксения и другие пользователи с Google теряют Composio до первого `/google` (один раз переподключиться)
- Включить лимит 5h rolling window (25 msg/5h) для paid — код не написан, только обсудили
