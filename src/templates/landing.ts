...
<meta name="description" content="Как Proboi защищает ваши данные: изоляция окружений, сетевая сегментация, управление секретами, безопасность платежей." />
<link rel="preconnect" href="https://fonts.googleapis.com" />
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
<link href="https://fonts.googleapis.com/css2?family=Unbounded:wght@600;700;800&family=Onest:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet" />
<style>
...
<div class="s-hero">
  ...
  <div class="s-hero__diagram">
    ...
    <svg width="320" height="320" viewBox="0 0 320 320" fill="none" aria-label="Слои изоляции Proboi">
      ...
      <!-- Метки колец -->
      <text x="160" y="18" text-anchor="middle" font-family="Onest, sans-serif" font-size="10" fill="var(--muted)">Сеть</text>
      <text x="160" y="50" text-anchor="middle" font-family="Onest, sans-serif" font-size="10" fill="var(--ink-2)">Защита ядра</text>
      <text x="160" y="79" text-anchor="middle" font-family="Onest, sans-serif" font-size="10" fill="var(--ink)">Изоляция</text>
      <text x="160" y="107" text-anchor="middle" font-family="Onest, sans-serif" font-size="10" fill="var(--ink)">Ресурсы</text>
...
<div class="s-section">
  ...
  <div class="s-layers">
    ...
    <div class="s-layer">
      ...
      <div class="s-layer__title">Изоляция окружений</div>
      <div class="s-layer__desc">Каждый пользователь работает в выделенном окружении. Никакой общей файловой системы, процессов или сети между пользователями нет.</div>
    </div>
    ...
    <div class="s-layer">
      ...
      <div class="s-layer__title">Защита ядра</div>
      <div class="s-layer__desc">Технология Google, применяемая в Google Cloud Run. Системные вызовы обрабатываются в безопасном промежуточном слое.</div>
    </div>
    ...
    <div class="s-layer">
      ...
      <div class="s-layer__desc">Два независимых уровня сетевой фильтрации. Пользователи изолированы друг от друга и от внутренней инфраструктуры.</div>
    </div>
    ...
    <div class="s-layer">
      ...
      <div class="s-layer__desc">Жёсткие лимиты по памяти, процессам и диску. Действие одного пользователя не влияет на остальных.</div>
    </div>
    ...
    <div class="s-layer">
      ...
      <div class="s-layer__desc">Токены и ключи API автоматически маскируются перед записью в журналы. Журналы недоступны извне инфраструктуры.</div>
    </div>
    ...
    <div class="s-layer">
      ...
      <div class="s-layer__desc">Регулярные циклы проверки безопасности с документированным реестром найденных и закрытых проблем.</div>
    </div>
...
<div class="s-section">
  ...
  <div class="s-flow">
    <svg width="860" height="160" viewBox="0 0 860 160" fill="none" aria-label="Схема прохождения запроса">
      ...
      <text x="622" y="68" text-anchor="middle" font-family="Onest, sans-serif" font-size="10" fill="var(--muted)">API без обучения на данных</text>
...
<div class="s-prose">
  <h3>Шаг 1 — Шифрование в пути</h3>
  <p>Сообщение идёт от вашего устройства до серверов Telegram по TLS. Содержание зашифровано в транзите. Telegram хранит и доставляет сообщения — это общий предел для всех сервисов на базе Telegram Bot API.</p>

  <h3>Шаг 2 — Аутентификация</h3>
  <p>Бот проверяет идентификатор отправителя по явному списку. Сообщения от неизвестных аккаунтов отбрасываются до любой обработки — без ответа пользователю, без записи в журнал.</p>

  <h3>Шаг 3 — Изоляция контекста</h3>
  <p>Для каждого пользователя создаётся отдельная сессия. История, память и настройки одного пользователя физически недоступны другому.</p>

  <h3>Шаг 4 — Маскирование перед записью</h3>
  <p>До записи в журнал ключи API автоматически заменяются плейсхолдерами. Журнал фиксирует факт запроса, а не его секретное содержимое.</p>

  <h3>Шаг 5 — Передача в модель</h3>
  <p>Запрос уходит к поставщику языковой модели. Все поставщики, с которыми работает Proboi, не используют API-запросы для обучения моделей — это зафиксировано в условиях их API-доступа. Браузерные интерфейсы и API — разные режимы с разными условиями.</p>
</div>
...
<div class="s-section">
  ...
  <div class="s-limits">
    ...
    <div class="s-limit">
      ...
      <div class="s-limit__text">Слоистая архитектура снижает вероятность успешной атаки и ограничивает её масштаб. Но 100% защиту от неизвестных уязвимостей не предоставляет никто, включая крупнейших облачных провайдеров.</div>
    </div>
    <div class="s-limit">
...
<div class="s-section">
  ...
  <div class="s-prose" style="margin-bottom:0">
    <p>Серверы Proboi работают на Timeweb Cloud в Москве. Пользовательские данные — история, файлы, настройки — хранятся в российской юрисдикции. Хостинг соответствует требованиям <strong>242-ФЗ</strong> о локализации персональных данных российских граждан.</p>
    <p>При этом сами запросы к языковым моделям уходят к внешним поставщикам за рубежом — это неизбежно для любого AI-сервиса. Для задач, где это принципиально, не передавайте через Proboi персональные данные.</p>
  </div>
</div>

<!-- ── О ПРОЦЕССЕ АУДИТА ──────────────────────────── -->
<div class="s-section">
  <h2 class="s-section__heading">Аудит безопасности</h2>
  <div class="s-prose" style="margin-bottom:0">
    <p>Аудит безопасности проводится регулярно. По итогам последнего цикла (май 2026) идентифицировано и устранено 27 пунктов разного уровня приоритета. Реестр проблем документирован: описание, принятая мера, подтверждение закрытия.</p>
    <p>Доступность сервиса контролируется независимым внешним монитором каждые 5 минут. Проблемы на самом сервере его не заглушат — сигнал приходит раньше, чем первый пользователь напишет в поддержку.</p>
  </div>
</div>

<!-- ── ОПЛАТА ──────────────────────────────────────── -->
<div class="s-section">
  <h2 class="s-section__heading">Безопасность платежей</h2>
  <div class="s-prose">
    <p>Оплата обрабатывается через <strong>ЮКассу</strong> — российский платёжный сервис группы Яндекс с лицензией ЦБ РФ (НКО № 3538-К). Мы не принимаем и не храним данные банковских карт самостоятельно.</p>
    <p>Когда вы нажимаете «оплатить», Telegram открывает платёжную форму ЮКассы прямо внутри приложения. Номер карты, CVV, срок действия — всё это вводится в интерфейс ЮКассы и никогда не проходит через наши серверы. Мы получаем только подтверждение: «платёж прошёл» или «платёж отклонён», и токен для управления подпиской.</p>
  </div>
  <div class="s-tech" style="margin:24px 0">Что мы видим после оплаты:
  payment_id · статус · сумма · дата

  Что мы никогда не видим:
  номер карты · CVV · имя держателя · 3DS-данные</div>
  <div class="s-prose" style="margin-bottom:0">
    <p>ЮКасса работает по стандарту <strong>PCI DSS</strong> — это международная сертификация для организаций, которые обрабатывают карточные данные. Сертификат означает: данные карты хранятся в зашифрованном виде в защищённой инфраструктуре с регулярными аудитами.</p>
    <p>Для отмены подписки или возврата — пишите в бот. Возвраты обрабатываются через тот же платёжный шлюз: деньги идут обратно тем же путём, которым пришли.</p>
  </div>
</div>
