...
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width,initial-scale=1" />
<title>Как настроить помощника — О, мойКлод</title>
<meta name="description" content="Как настроить ИИ-ассистента в Telegram: познакомить с собой, загрузить документы, подключить почту и календарь, поставить расписание." />
${HEAD_LINKS}
<style>
  .guide { max-width: 760px; margin: 0 auto; padding: 64px var(--pad); }
  .guide h2 { font-family: var(--f-display); font-weight: 700; font-size: clamp(24px, 3vw, 36px); margin: 48px 0 12px; letter-spacing: -.02em; }
  .guide p { color: var(--ink-2); font-size: 17px; line-height: 1.55; max-width: 60ch; }
  .guide ol { padding-left: 22px; }
  .guide li { margin-bottom: 8px; }
  .guide__draft {
    display: inline-block; background: var(--coral); color: var(--cream);
    font-family: var(--f-mono); font-size: 12px; padding: 4px 10px; border-radius: 999px; margin-bottom: 18px;
  ...
  .guide__realguide a { color: var(--coral); text-decoration: underline; text-underline-offset: 3px; font-weight: 600; }
</style>
</head>
<body>
...
${NAV_HTML.replace('href="/"', 'href="/"').replace('href="/#features"', 'href="/#features"').replace('href="/#faq"', 'href="/#faq"')}

<main class="guide">
  <span class="guide__draft">черновик</span>
  <h1 class="display display--md">Как настроить<br/><span class="ink-stroke">помощника.</span></h1>
  <p>Эта страница ещё в работе. Пока — план содержания и ссылка на боевой гайд с реальными сценариями.</p>

  <div class="guide__realguide">
    Настоящий гайд с&nbsp;примерами уже есть на GitHub: <a href="${REAL_GUIDE_URL}" target="_blank" rel="noopener">guide/ru →</a>.
    Здесь скоро будет аккуратная версия с&nbsp;картинками и&nbsp;шагами.
  </div>

  <h2>1. Познакомить бота с собой</h2>
  <p>Что написать в первом сообщении, что рассказать о себе и&nbsp;своих задачах.</p>

  <h2>2. Загрузить документы в&nbsp;личную библиотеку</h2>
  <p>Какие форматы поддерживаются, как организовать «ящики», как сослаться потом.</p>

  <h2>3. Подключить почту, календарь, заметки</h2>
  <p>Один раз нажал «разрешить», что бот теперь умеет, как отозвать доступ.</p>

  <h2>4. Поставить расписание</h2>
  <p>Примеры фраз, которые понимает бот: «каждое утро», «по&nbsp;понедельникам в&nbsp;9», «если...».</p>

  <h2>5. Поручить большую задачу и&nbsp;забрать утром</h2>
  <p>Как формулировать долгие задачи и&nbsp;где искать результат.</p>

  <h2>6. Если бот забыл что-то важное</h2>
  <p>Как явно сказать «запомни это», как править узлы графа.</p>

  <h2>7. Удалить всё и&nbsp;начать с&nbsp;нуля</h2>
  <p>Одной фразой. Без отдела поддержки.</p>
