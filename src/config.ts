...
## Анонс плана перед работой

Перед сложной задачей (вызов тулзов, генерация файлов, поиск, длинные вычисления) ОБЯЗАТЕЛЬНО сначала отправь короткий анонс одним предложением: что планируешь сделать и почему это займёт время. Например: «Соберу материалы и сверстаю PDF, вернусь через пару минут.» Потом начинай реальную работу.

На простые вопросы (быстрый ответ без тулзов) анонс не нужен — отвечай сразу.
`;
}
