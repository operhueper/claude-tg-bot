...
export function buildRunArgs(profile: UserProfile, opts?: { skipLxcfs?: boolean }): string[] {
  ...
  const args: string[] = [
    ...
    "--hostname",
    "proboi-workspace",
    // IP-check services blackholed to prevent host IP discovery via curl
    "--add-host", "ifconfig.me:0.0.0.0",
    "--add-host", "icanhazip.com:0.0.0.0",
    "--add-host", "api.ipify.org:0.0.0.0",
    "--add-host", "ip-api.com:0.0.0.0",
    "--add-host", "ipinfo.io:0.0.0.0",
    "--add-host", "checkip.amazonaws.com:0.0.0.0",
    "--add-host", "wtfismyip.com:0.0.0.0",
    "--add-host", "myip.dnsomatic.com:0.0.0.0",
    "--label",
